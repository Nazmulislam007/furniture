...
export default async function handler(req, res) {
	try {
		...
		if (customerId) {
			const querySql =
				"SELECT * FROM my_price WHERE customer_id = " +
				customerId +
				" AND category_id = " +
				category_id;
			...
			if (response.length) {
				const updateSql =
					"UPDATE my_price SET sign = ?, price = ? WHERE category_id = ? AND customer_id = ?";
				const valueParams = [sign, price, category_id, customerId];
				const data = await query({ query: updateSql, values: valueParams });
				res.status(200).json({ data });
			} else {
				const insertSql =
					"INSERT INTO my_price (`id`, `sign`, `price`, `category_id`, `customer_id`) VALUES (?,?,?,?,?)";
