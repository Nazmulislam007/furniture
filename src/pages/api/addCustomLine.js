...
export default async function handler(req, res) {
    try {
        ...
        if (product?.id) {
            if (id) {
                var querySql = "UPDATE `custom_line_items` SET `name` = '" + name + "' , `price` = '" + parseFloat(value) + "' WHERE `id` = '" + id + "'";
                const valueParams = [];
                const data = await query({ query: querySql, values: valueParams });
                res.status(200).json({ id: data.insertId });
            } else {
                var querySql = "INSERT INTO custom_line_items (`name`, `category_id`, `customer_id`, `contractor_id`, `price`) VALUES (?,?,?,?,?)";
