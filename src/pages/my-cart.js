...
import { addNewLineItem, getCartByCustomer, getCategory, getCustomeDiscount, getCustomeLineItems, getMyPrice, removeLineItem } from '@/Context/utility';
...
export default function MyCart({ customerId }) {
   ...
   const getAllCustomDiscount = async () => {
      if (CUSTOMER_ID) {
         const customeLineData = await getCustomeDiscount({ customerId: CUSTOMER_ID });
   ...
   const getCustomLine = async () => {
      ...
      const customeLineData = await getCustomeLineItems({ customerId: CUSTOMER_ID });
      let cate = {};
      newArr.map((elem) => {
         ...
         cartData.map((ele) => {
            myPriceArr.find((x) => {
               if (x.category_id === ele.category_id) {
                  if (x?.sign === "%") {
                     return ele.myprice = (parseFloat(ele?.price) / 100) * parseFloat(x?.price);
                  } else {
   ...
   const subTotal = (+serviceTotalMyCart) + (+totalProductAmount) + (+totalCustomLine) - (+totalDiscountLine);
   ...
   return (
      <Box maxWidth="md" marginX="auto" px={1} mb={5} mt={2}>
         <Box sx={{ padding: 4, bgcolor: 'white' }}>
            ...
            {(cartItems) && cartItems.map(([key, values]) => {
               values.type = key;
                     ...
                     {values.length > 0 && (
                        <>
                           ...
                           <Stack py={3} spacing={2} borderBottom="2px solid">
                              {(customLineItem[key]) && customLineItem[key].map((lineItem) => (
                                 <>
                                    <Stack key={lineItem.id} direction="row" justifyContent="space-between">
                                       <div>
                                          <input type='text' onChange={(e) => { onChangeHandler(key, `name`, e.target.value, lineItem) }} name={`name_${key}`} value={lineItem.name} />
                                       </div>
                                       <div>
                                          <input type='text' onChange={(e) => { onChangeHandler(key, `price`, e.target.value, lineItem) }} name={`value_${key}`} value={lineItem.price} />
                                          <button onClick={() => saveLineItem(lineItem, values.type, key)} title='Save'><DoneIcon style={{ color: "green" }} /></button>
                                          <button onClick={() => removeCustomLine(lineItem, values.type, key)} title='Remove'><CloseIcon style={{ color: "red" }} /></button>
                                       </div>
                                    ...
                                    return (
                                       <>
                                          <Stack direction="row" justifyContent="space-between">
                                             <div>
                                                <input type='text' onChange={(e) => { onChangeHandler(index, `name`, e.target.value, false) }} name={`name_${index}`} value={customObj?.name} placeholder='*Custom Line Item*' />
                                             </div>
                                             <div>
                                                <input type='text' onChange={(e) => { onChangeHandler(index, `value`, e.target.value, false) }} name={`value_${index}`} value={customObj?.price && 0} placeholder='0' />
                                                <button onClick={() => saveLineItem(customObj[index], values.type, index)} title='Save'><DoneIcon style={{ color: "green" }} /></button>
                                                <button onClick={() => removeCustomLine(customObj[index], values.type, index)} title='Remove'><CloseIcon style={{ color: "red" }} /></button>
                                             </div>
                                          </Stack>
                                       </>)
                                 }
                              ...
                              <Button variant="text" startIcon={<AddIcon />} sx={{ color: 'gray' }} onClick={() => addCustomLineItem(values.type,)}>
                                 Add Line Item
                              </Button>
                           </Stack>
                        </>
