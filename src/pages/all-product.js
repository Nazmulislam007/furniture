...
import { getCategory, getMyPrice, getProductByCategory, insertMyPrice } from '@/Context/utility';
...
export default function AllProduct() {
  ...
  const getAllProducts = async () => {
    ...
    const newArr = [];
    category.filter((x) => newArr.push(x.url));
    setCatagory(newArr);
  ...
  return (
    <Box maxWidth="lg" marginX="auto" px={1} mb={5} mt={2}>
      <Box sx={{ width: 'fit-content', marginInline: 'auto' }}>
        ...
        <Stack direction="row" gap={2} display="flex" flexWrap="wrap" justifyContent="center">
          <Stack
            bgcolor="white"
            height="fit-content"
            padding={1}
          ...
          >
            <Typography component="p" fontSize={22} padding="10px 10px 0px 10px">
              All Catagory
            </Typography>
            <MenuList>
              {catagory.map((cata, i) => (
              ...
              <Stack>
                ...
                <Stack direction="row" gap="10px" justifyContent="center">
                  <Stack component="form" width={50}>
                    <TextInput value={myPrice?.price} name="price" onChange={handleChange} />
                  </Stack>
                  <SelectDropDown
                    menuItems={['$', '%']}
                    name="sign"
