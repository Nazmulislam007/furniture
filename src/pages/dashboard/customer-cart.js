...
export default function CustomerCart() {
   ...
   const totalProductAmount = Object.keys(addtoCart[CUSTOMER_ID].product).reduce((accumulator, currentKey) => {
      const items = addtoCart[CUSTOMER_ID].product[currentKey];
      if (items && items.length > 0) {
         return accumulator + items.reduce((sum, item) => (sum + item.totalPrice ? +item.totalPrice : +item.price), 0);
      }
      return accumulator;
   }, 0);
   ...
   return (
      <Box maxWidth="lg" marginX="auto" px={1} mb={5} mt={2}>
         <Stack direction="row" gap={2} display="flex" flexWrap={{ lg: 'nowrap', xs: 'wrap' }} justifyContent="center">
            <CustomerList />
            <Stack spacing={2}>
