...
export default function ServiceCost({ info, setServiceTotal }) {
   ...
   const getServiceCostData = async () => {
      const serviceCostData = await getServiceCost({ customerId: info.CUSTOMER_ID });
      setServiceCost1(serviceCostData);
   };

   let serviceType = info.type;
   let totalSqft;

   if (info.length === 1) totalSqft = info[0].customerSqft;
   if (info.length > 1) totalSqft = info.reduce((prev, next) => prev + next.customerSqft, 0);

   const service = serviceCost1?.find(
      (item) => item?.service.toLowerCase() === serviceType.toLowerCase() && item.customer_id === info.CUSTOMER_ID
   );
   ...
   if (service) {
      serviceInstall = (service) && (service.install ? JSON.parse(service.install) : "")
      serviceRemove = (service) && (service.remove ? JSON.parse(service.remove) : "")
      serviceDeliver = (service) && (service.deliver ? JSON.parse(service.deliver) : "")
         ...
         if (arr) {
            for (let i = 0; i < arr.length; i++) {
               ...
               if (obj.totalSqft <= totalSqft) {
                  if (closestObj == null || totalSqft - obj.totalSqft < totalSqft - closestObj.totalSqft) {
                     closestObj = obj;
                  }
               } else if (closestObj == null || obj.totalSqft - totalSqft < closestObj.totalSqft - totalSqft) {
                  closestObj = obj;
               }
            }
         }
      ...
      closestObjInstall = findClosestObj(serviceInstall[Object.keys(serviceInstall)[0]]);
      closestObjRemove = findClosestObj(serviceRemove[Object.keys(serviceRemove)[0]]);
      closestObjDliver = findClosestObj(serviceDeliver[Object.keys(serviceDeliver)[0]]);
   }
   const findServiceType = (type, closestObj) => {
      ...
      if (Object.keys(serv)[0] === 'Square Foot') {
         return (
            <> {Object.values(serv)[0].length > 0 && Object.values(serv)[0].map((ele) => {
               return (
                  <>
                     <Box>
                        {ele.totalSqft}{' '}
   ...
   return (
      ...
      >
         ...
         <Stack sx={{ display: 'grid', gap: 2, gridTemplateColumns: 'repeat(2, 1fr)', fontWeight: 500 }}>
            <Box>Installation costs</Box>
            {totalSqft ? <Box sx={{ justifySelf: 'flex-end' }}>{totalSqft} sq.ft.</Box> : <Box />}
            <Box>Removal</Box>
            {totalSqft ? <Box sx={{ justifySelf: 'flex-end' }}>{totalSqft} sq.ft.</Box> : <Box />}
            <Box>Delivery</Box>
            {totalSqft ? <Box sx={{ justifySelf: 'flex-end' }}>{totalSqft} sq.ft.</Box> : <Box />}
         </Stack>
         <Stack sx={{ display: 'grid', gap: 2, gridTemplateColumns: 'repeat(2, 1fr)', fontWeight: 500, fontSize: 20 }}>
