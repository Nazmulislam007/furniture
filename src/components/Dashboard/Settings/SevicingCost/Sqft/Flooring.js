...
const SERVICE = 'Flooring';

export default function FlooringCost({ setCategory, setSettings }) {

   const CUSTOMER_ID = JSON.parse(sessionStorage.user).customer_id;
   const [userData, setUserData] = useState({});
   const [data, setData] = useState({});

   const { tilesGobalState, dispatchServiceCost, serviceCost, dispatch } = useGlobalState() || {};
   ...
   useEffect(() => {
      ...
      getServiceCostData({ selected: 'flooring', customerId: userData?.customer_id })
   }, []);
      ...
      data[key] = {
         [tilesGobalState.answer[key]]:
            tilesGobalState.answer[key] === 'Square Foot' ?
               (Array.isArray(tilesGobalState.incrementBoxes[key]) ? [...tilesGobalState.incrementBoxes[key]] : tilesGobalState.incrementBoxes[key])
   ...
   const getServiceCostData = async (obj) => {
      ...
      if (result && result.length > 0) {
         ...
         const servicecostobj = { deliver, dispose, install, service, customerId, categoryId, remove: { Free: 0 }, underlayment: { Free: 0 } }
   ...
   const handleClick = async () => {
      ...
      dispatchServiceCost({
         type: 'SET_SERVICES',
         payload: { ...tilesGobalState, service: SERVICE, customerId: userData?.id },
      });
   };
      ...
      <Stack spacing={3} sx={{ bgcolor: 'white', padding: 4, flex: 1, width: '100%' }}>
         <Stack>
            <Typography component="p" fontSize={22} fontWeight="500">
               Flooring - Service Costs
            </Typography>
            <Typography component="span" fontSize={14} color="gray">
               Settings for your Flooring options
            </Typography>
         </Stack>
         <List sx={{ display: 'flex', flexDirection: 'column' }}>
            {questions.map((q, i) => (
               <Question quesitionName={q.question} questionId={q.questionId} key={i} />
            ))}
         </List>
