...
const SERVICE = 'Shower Kit';
const options = ['Free', 'Flat Rate', 'Not Available'];

export default function ShowerKitCost({setCategory,setSettings}) {
   ...
   const getServiceCostData = async (obj) => {
     ...
     if(result.length > 0){ const install = JSON.parse(result[0].install)
      ...
      const newData = { install, service, customerId, categoryId, deliver, removeBathtub, removeShower,dispose }
   ...
   useEffect(() => {
      ...
      getServiceCostData({ selected: 'showerkit', customerId: userData?.customer_id })
   }, [])
 
   useEffect(() => {
      if (responseData && (inputVal.install !== '' && inputVal.removeShower !== '')) {
   ...
   const data = {
      ...
      removeBathtub: { [question.removeBathtub]: question.remove !== 'Flat Rate' ? 0 : inputVal.removeBathtub },
      removeShower: { [question.removeShower]: question.remove !== 'Flat Rate' ? 0 : inputVal.removeShower },
      ...
      <Stack spacing={3} sx={{ bgcolor: 'white', padding: 4, flex: 1, width: '100%' }}>
         <Stack>
            <Typography component="p" fontSize={22} fontWeight="500">
               Shower Kit - Service Costs
            </Typography>
            <Typography component="span" fontSize={14} color="gray">
               Settings for your Shower Kit options
            </Typography>
         </Stack>
         <List sx={{ display: 'flex', flexDirection: 'column' }}>
            <Question quesitionName="deliver a new shower kit?">
            ...
            <Question quesitionName="install a new shower kit?">
            ...
            <Question quesitionName="remove an existing bathtub?">
            ...
            <Question quesitionName="remove an existing shower?">
            ...
            <Question quesitionName="dispose an existing bathtub?">
