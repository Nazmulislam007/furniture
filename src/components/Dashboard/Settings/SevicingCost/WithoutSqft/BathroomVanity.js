...
export default function BathroomVanityCost({ setSettings, setCategory }) {
   ...
   const getServiceCostData = async (obj) => {
      ...
      if (result.length > 0) {
         ...
         const newData = { deliver, dispose, install, service, customerId, categoryId, remove, }
   ...
   useEffect(() => {
      ...
      getServiceCostData({ selected: 'vanities', customerId: userData?.customer_id })
   }, [])
   ...
   const data = {
      ...
      remove: { [question.remove]: question.remove !== 'Flat Rate' ? 0 : inputVal.remove },
