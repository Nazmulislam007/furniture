...
import { Button, InputLabel, Stack, TextareaAutosize, Typography } from '@mui/material';
...
const pages = ['Home', 'Flooring', 'Tiles', 'CounterTop', 'Vanities', 'Kitchen Faucets', 'Bathroom Faucets', 'Cabinets'];

export default function Form({ customer }) {
   const currentUser = typeof window !== "undefined" ? JSON.parse(window.sessionStorage.getItem('user')) : null;
   ...
   const formik = useFormik({
      ...
      onSubmit: async (values) => {
         ...
         if (customers?.find((customer) => customer.id === id)) return toast('Already user exist', 'red').showToast();
         /* dispatch(
            createCustomer({
               ...values,
   ...
   const selectedStateCode = State.getStatesOfCountry('US').find((state) => state.name === formik.values.state);
   const cityName = City.getCitiesOfState('US', selectedStateCode?.isoCode).map((name) => name.name);
   ...
   return (
      ...
      >
         ...
         <Stack>
            ...
            <Stack component="button" type="button" direction="row" ref={copyIdRef} alignItems="center" spacing={1}>
         ...
         />
         <Button type="submit" variant="contained" sx={{ width: 'fit-content', gridColumn: 2 / -1, justifySelf: 'end' }}>
            Create
         </Button>
      </Stack>
   );
