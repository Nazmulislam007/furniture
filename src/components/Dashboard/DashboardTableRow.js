...
export default function DashboardTableRow({ cus, dispatch }) {
  ...
  useEffect(() => {
    async function getUrl() {
      try {
        ...
        if (data.length > 0) {
          setUniqueUrl(`${window.location.origin}/${data[0].url}/${data[0].customer_id}`);

          const page = selectItems.find((item) => item.url === data[0].landingpage_id);
  ...
  const handleSelect = async (e) => {
    if (!currentUser?.id) return alert("Please login to add toggle status.");
    const { page } = selectItems.find((item) => item.url === e.target.value);
  ...
  const handleCopyClick = async () => {
    try {
      if (navigator.clipboard && navigator.clipboard.writeText) {
        const { textContent } = copyIdRef.current;
        if (textContent) {
      ...
      <TableCell>
        <Stack>
          <Stack direction="row" spacing={1} alignItems="center">
            <Box ref={copyIdRef} sx={{ maxWidth: "170px", mr: "10px", display: "table" }}>
              {uniqueUrl}
              <CopyAllIcon
