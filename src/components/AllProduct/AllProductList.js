...
export default function AllProductList({ products, selected, setProductId }) {
   ...
   const filterProduct = products && products.filter((prod) => {
         ...
         <Stack>
            ...
            <TableContainer>
               <Table>
                  ...
                  <TableBody>
                     {loadProduct && loadProduct.map((product) => (
                        <TableRow key={product.id} sx={{ '&:last-child td, &:last-child th': { border: 0 } }}>
                           <CustomTableCell width="70px">
                              <StatusBtn setProductId={setProductId} product={product} selected={selected} />
                           </CustomTableCell>
                           <CustomTableCell>
                           ...
                           <CustomTableCell sx={{ textAlign: 'end', fontWeight: 500 }}>${product.price}</CustomTableCell>
                        </TableRow>
                     ))}
                  </TableBody>
               </Table>
