...
export default function MiniSlider({ products }) {
	...
	const { dispatch, filterByPrice, storeFilterOptions } =
		useSeletedProduct() || {};
	...
	const filterAllProduct = () => {
		...
		if (filterByPrice.min) {
			filterProducts = filterProducts?.filter(
				(prod) => prod.price > filterByPrice.min,
			);
		}

		if (filterByPrice.max) {
			filterProducts = filterProducts?.filter((prod) =>
				filterByPrice.max === "" ? prod : prod.price < filterByPrice.max,
			);
		}

		if (storeFilterOptions?.length > 0) {
			filterProducts = filterProducts?.filter((prod) =>
				storeFilterOptions?.every((option) =>
					Object.keys(option).every((key) => prod[key] === option[key]),
				),
			);
		}
