...
import { getCartByCustomer, getCategory, getMyPrice, updateCartQty } from '@/Context/utility';
import { Box, Button, Stack, Typography } from '@mui/material';
import { useEffect, useState } from 'react';

export default function PriceCalculation({ name }) {
   ...
   const handleSubmit = (e) => {
      ...
      setShowResult({
         ...
         totalPrice: Math.ceil(inputSqft / Number(sqft)) * Number(sqft) * Number(selectedProduct.price),
      });
   };
   ...
   const myPrice = (mySetPrice) && mySetPrice?.sign === '%' ? ((selectedProduct?.price / 100) * mySetPrice?.price).toFixed(2) : mySetPrice?.price;
   ...
   const handleCalculate = (e) => {
      ...
      updateCartQty({ name: "", quantity: quantity, id: isExistProduct.id, total: total });
   }
   ...
   const handleQuantity = (quantity, type) => {
      ...
      if (newQuantity > 0) {
         ...
         const subTotal = (total / (newQuantity + TAX_PERCENT / 100)).toFixed(2) || 0
         setTotalPrice(total)
         setSubTotalPrice(subTotal)
         updateCartQty({ name: "", quantity: newQuantity, id: isExistProduct.id, total: total });
      }
   }
   return (
